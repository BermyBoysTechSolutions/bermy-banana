...
const serverEnvSchema = z.object({
  ...
  // AI - OpenRouter (for Script Assistant)
  OPENROUTER_API_KEY: z.string().optional(),
  OPENROUTER_MODEL: z.string().default("openai/gpt-5-mini"),

  // AI - Google Gemini (for Veo 3.1 video and Nano Banana image generation)
...
export function checkEnv(): void {
  ...
  if (!process.env.OPENROUTER_API_KEY) {
    warnings.push("OPENROUTER_API_KEY is not set. Script Assistant will not work.");
  }

  if (!process.env.GOOGLE_AI_API_KEY) {
    warnings.push("GOOGLE_AI_API_KEY is not set. Video/image generation will not work.");
  }
